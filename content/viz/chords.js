...
var colors = ["#C0C0C0","#E6C230","#EC1D25","#BC2697","#26BC97","#E13051","#F36534","#737373"];
																			//
...
function Draw1(){
	...
	loc = 8/2, delayDisappear = 1, delayAppear = 2, finalText = true);
...
function Draw2(){ 
	...
	d3.selectAll("g.group").selectAll("line")
		...
		.style("stroke", function(d, i, j) {return j ? 0 : "#c0c0c0"; });
...
function Draw3(){
   ...
   svg.selectAll("g.group").select("path")
	.transition().delay(function(d, i) { return 100*arcDelay[i];}).duration(100)
	.attrTween("d", function(d) {
		if(d.index != 0) {
		   var i = d3.interpolate(d.startAngle, d.endAngle);
		   return function(t) {
  ...
  svg.selectAll("g.group")
	.transition().delay(function(d,i) { return 100*arcDelay[i]; }).duration(100)
	.selectAll("g").selectAll("line").style("stroke", "#C0C0C0");
  /*Same for the %'s*/
  svg.selectAll("g.group")
	.transition().delay(function(d,i) { return 100*arcDelay[i]; }).duration(100)
...
function Draw4(){
	...
	d3.selectAll("g.group").selectAll("line")
		.transition().duration(100)
		.style("stroke",function(d,i,j) {if (j == 5 || j == 4) {return "#242424";} else {return "#c0c0c0";}});
	/*Same for the %'s*/
	svg.selectAll("g.group")
		.transition().duration(100)
...
function Draw10(){
	...
	d3.selectAll("g.group").selectAll("line")
		.transition().duration(200)
		.style("stroke",function(d,i,j) {if (j == 4) {return "#C0C0C0";} else {return "#242424";}});
	/*Same for the %'s*/
	svg.selectAll("g.group")
		.transition().duration(200)
...
function Draw11(){
	...
	d3.selectAll("g.group").selectAll("line")
		.transition().duration(200)
		.style("stroke",function(d,i,j) {if (j == 0) {return "#c0c0c0";} else {return "#242424";}});
	/*Same for the %'s*/
	svg.selectAll("g.group")
		.transition().duration(200)
...
function finalChord() {
	...
	svg.selectAll("g.group")
		.transition().duration(100)
		.selectAll(".tickLabels").style("opacity",1).style("fill","#c0c0c0");
	/*And the Names of each Arc*/	
	svg.selectAll("g.group")
		.transition().duration(100)
		.selectAll(".titles").style("opacity",1);		
...
function wrap(text, width) {
    var text = d3.select(this)[0][0],
        ...
        tspan = text.text(null).append("tspan").attr("fill","#C0C0C0").attr("x", x).attr("y", y).attr("dy", dy + "em");
    ...
    while (word = words.pop()) {
      ...
      if (tspan.node().getComputedTextLength() > width) {
        ...
        tspan.text(line.join(" "));
        line = [word];
        tspan = text.append("tspan").attr("fill","#C0C0C0").attr("x", x).attr("y", y).attr("dy", ++lineNumber * lineHeight + dy + "em").text(word);
      };
    };
};
